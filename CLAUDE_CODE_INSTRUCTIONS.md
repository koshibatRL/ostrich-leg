...
## ⚠️ CRITICAL: Model A 膝方向の修正（2026-03-28）

**Model Aの膝関節rangeに致命的な設計ミスがあり、修正済み。**

### 何が間違っていたか
`models/model_a_forward_knee.xml` の膝rangeが `"-120 0"` になっていた。
これはModel B（ダチョウ型）と同じ方向に曲がる設定であり、
Model A（人間型）とModel Bが実質同じ膝方向になっていた。
→ **A vs B の比較実験として成立していなかった。**

### 修正内容（コミット済み）
- `models/model_a_forward_knee.xml`: 両膝の `range="-120 0"` → `range="0 120"`
- `configs/model_a_config.py`: `r_knee`/`l_knee` のデフォルト位置 `-0.4` → `0.4`

### MuJoCoでの膝方向の解釈（`axis="0 1 0"` の場合）
- `range=[-120, 0]` + 負の角度 → 脛が進行方向の前方に出る（ダチョウ型）
- `range=[0, 120]` + 正の角度 → 脛が進行方向の後方に出る（人間型）

### H200でやるべきこと
1. **最新のリポジトリをpullする**（`git pull origin main`）
2. **Model Aの既存の訓練結果がある場合、破棄する**（修正前のXMLで訓練された結果は無効）
3. **Model Aのみ再訓練を実行する**
4. **Model B, Cは影響なし。** 既存の訓練結果はそのまま使える

### 検証用動画
`docs/videos/` に各モデルの膝可動域sweep動画を追加済み。
修正後の方向が正しいことを可視化で確認済み。
